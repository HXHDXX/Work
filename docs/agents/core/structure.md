...
│               └── cpp-env.md
├── libs/                        ← 项目级私有库（按需创建）
├── cross-library-examples/      ← 开源库使用示例（按需创建）
├── container/
│   └── build.sh                 ← 统一构建验证脚本
...
| 目录 | 用途 | 规则 |
|------|------|------|
| `libs/` | 项目级私有库 | 按需创建，每个库一个子目录 |
| `cross-library-examples/` | 开源库使用示例 | 每个库一个子目录，含 CMakeLists 示例 |
| `container/` | 构建与部署脚本 | `build.sh` 统一入口 |
...
1. **代码模块** → 放在根目录或 `libs/` 下，遵循 CMake 项目结构
2. **第三方依赖** → 优先系统安装（`/opt/cross-library/`），其次 `libs/`
3. **开源库示例** → `cross-library-examples/{library-name}/`
4. **测试** → 与代码模块同目录的 `tests/` 子目录
5. **配置文件** → 与模块同目录，不集中管理
...
- [ ] 第三方依赖遵循优先级：C++ 标准库 → 开源库 → 私有库
- [ ] `libs/` 中的库有对应的 `cross-library-examples/` 示例（如适用）
- [ ] 知识库已更新（非 trivial 变更）
